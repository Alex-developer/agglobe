...
void    scan_map(void);
void    orth_scan_outline(void);
void    orth_scan_curves(void);
double *orth_extract_curve(int, short *);
void    orth_scan_along_curve(double *, double *, int);
void    orth_find_edge_xing(double *, double *, double *);
void    orth_handle_xings(void);
void    orth_scan_arc(double, double, double, double, double, double);
void    merc_scan_outline(void);
void    merc_scan_curves(void);
double *merc_extract_curve(int, short *);
void    merc_scan_along_curve(double *, double *, int);
double  merc_find_edge_xing(double *, double *);
void    merc_handle_xings(void);
void    merc_scan_edge(EdgeXing *, EdgeXing *);
void    xing_error(const char *, int, int, int, EdgeXing *);
void    scan(double, double, double, double);
void    get_scanbits(int);

static int double_comp(const void *, const void *);
static int scanbit_comp(const void *, const void *);
static int orth_edgexing_comp(const void *, const void *);
static int merc_edgexing_comp(const void *, const void *);
...
static int scanbit_comp(const void *a, const void *b)
{
  return (((const ScanBit *) a)->y - ((const ScanBit *) b)->y);
}


static int orth_edgexing_comp(const void *a, const void *b)
{
  double angle_a;
  double angle_b;
  int    rslt;
...
static int merc_edgexing_comp(const void *a, const void *b)
{
  double val_a;
  double val_b;
  int    rslt;
...
double *orth_extract_curve(int npts, short *data)
{
  int     i;
  int     x, y, z;
  double  scale;
...
void orth_scan_along_curve(double *prev, double *curr, int cidx)
{
  double    extra[3];
  EdgeXing *xing;
...
void orth_find_edge_xing(double *prev, double *curr, double *rslt)
{
  double tmp;
  double r0, r1;
...
void orth_scan_arc(double x_0, double y_0, double a_0, double x_1, double y_1, double a_1)
{
  int    i;
  int    lo, hi;
  double angle, step;
...
double *merc_extract_curve(int npts, short *data)
{
  int     i;
  int     x, y, z;
  double  scale;
...
void merc_scan_along_curve(double *prev, double *curr, int cidx)
{
  double    px, py;
  double    cx, cy;
  double    dx;
...
double merc_find_edge_xing(double *prev, double *curr)
{
  double ratio;
  double scale;
  double z1, z2;
...
void merc_scan_edge(EdgeXing *from, EdgeXing *to)
{
  int    s0, s1, s_new;
...
void xing_error(const char *file, int line, int idx, int nxings, EdgeXing *xings)
{
  fflush(stdout);
...
void scan(double x_0, double y_0, double x_1, double y_1)
{
  int    i;
  int    lo_y, hi_y;
  double x_value;
