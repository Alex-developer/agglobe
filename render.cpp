...
static void new_stars(double);
static void new_grid(int, int);
static void new_grid_dot(double *, double *);
static int dot_comp(const void *, const void *);
static void render_rows_setup(void);
static void render_next_row(s8or32 *, int);
static void no_shade_row(s8or32 *, u_char *);
static void compute_sun_vector(double *);
static void orth_compute_inv_x(double *);
static void orth_shade_row(int, s8or32 *, double *, double *, u_char *);
static void merc_shade_row(int, s8or32 *, double *, u_char *);
...
static int dot_comp(const void *a, const void *b)
{
...
static void render_next_row(s8or32 *buf, int idx)
{
  int      i, i_lim;
  int      tmp;
  int      _scanbitcnt;
...
static void no_shade_row(s8or32 *scanbuf, u_char *rslt)
{
  int i, i_lim;
...
static void compute_sun_vector(double *rslt)
{
...
static void orth_compute_inv_x(double *inv_x)
{
  int i, i_lim;
...
static void orth_shade_row(int idx, s8or32 *scanbuf, double *sol, double *inv_x, u_char *rslt)
{
  int    i, i_lim;
  int    val;
  double x, y, z;
...
static void merc_shade_row(int idx, s8or32 *scanbuf, double *sol, u_char *rslt)
{
  int    i, i_lim;
  int    val;
  double x, y, z;
...
void render(void (*rowfunc)(u_char *))
{
  int     i, i_lim;
  s8or32 *scanbuf;
  u_char *row;
...
static void new_stars(double freq)
{
  int      i;
  int      x, y;
  int      max_stars;
...
static void new_grid(int big, int small)
{
  int    i, j;
  int    cnt;
  double lat, lon;
...
static void new_grid_dot(double *cs_lat, double *cs_lon)
{
  int      x, y;
  double   pos[3];
  ScanDot *news;
  ...
  {
    news = (ScanDot *) extarr_next(dots);
    news->x    = x;
    news->y    = y;
    news->type = DotTypeGrid;
  }
}
